...
## Source[^3]
- (1) A computer input device that resembles a typewriter keyboard but with some additional keys, which may include a control key, function keys, and arrow keys. The keyboard sends electrical signals (called scan codes) to the CPU when a user presses a key. 
- (2) An electronic musical instrument that incorporates a piano-style keyboard.
## References
