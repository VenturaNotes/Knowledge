...
## Source[^6]
- (probability) In probability, the complement of an event $A$ is the event 'not A', that is, the event of $A$ not occurring.
---
- (group theory) Let $H$ be a subgroup of a group $G$. A complement $K$ of $H$ is a subgroup such that$$ G=HK=\{hk : h \in H, k \in K\} $$and $H \cap K = \{e\}$. If such a complement exists, then every element can be uniquely written as $hk$. But a complement need not exist, nor be unique if one does.
---
- complementation Let $A$ be a subset of some universal set $E$. Then the complement of $A$ is the set difference $E \setminus A$ and may be denoted $A'$ or $A^c$ or $\bar{A}$ when the universal set is understood or has been specified. Complementation (the operation of taking the complement) is a unary operation on the power set of $E$. The following properties hold:
