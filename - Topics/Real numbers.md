...
- A real number can be defined in several ways, for example as the limit of a sequence of rational numbers. A real $x$ is represented by a sequence $q(0), q(1), \ldots$ of rational numbers that approximates $x$ in the sense that for any degree of accuracy $\varepsilon$ there exists some natural number $n$ such that$$\text { for all } k>n,|q(k)-x|<\varepsilon$$
- A real number is a computable real number if there is an algorithm that allows us to compute an approximation to the number to any given degree of accuracy. A real $x$ is computable if (a) there is an algorithm that lists a sequence $q(0), q(1), \ldots$ of rational numbers that converges to $x$, and (b) there is an algorithm that to any natural number $k$ finds a natural number $p(k)$ such that$$\text { for all } n>p(k), \mid q(n)-x \mid<2^{-k}$$
- Most of the real numbers that we know and use come from solving equations (e.g. the algebraic numbers) and evaluating equationally defined sequences (e.g. e and $\Pi$) and are computable. However, most real numbers are noncomputable.
