...
## Organize
-   **#question Why are they called tasks? What does task mean in this context?**
    In machine learning, a "task" refers to the specific problem that a machine learning model is designed to solve. It defines the objective and the type of output expected from the model.
    -   For **classification**, the task is to predict a discrete category or class label.
    -   For **regression**, the task is to predict a continuous numerical value.
    So, "task" in this context means the specific problem or objective that the supervised learning algorithm is trying to achieve.
