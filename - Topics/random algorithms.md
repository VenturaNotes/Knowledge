...
- An example is that of trying to test whether or not a number is prime. Given an integer $n$ there is a test that uses a 'guess' $i$, chosen at random between 1 and $n$, that takes $\mathrm{O}\left(\log _{2} n\right)$ time to perform. If the test is successful $n$ is known to have factors; on the other hand if $n$ has factors then the test will be successful for at least half of the integers in the range 1 to $n$. Thus if the test fails $k$ times it can be said that $n$ is prime with probability $1- 2^{-k}$.
