...
- (2) A logic circuit, usually an integrated circuit, that generates a unique $n$-bit binary word, indicating which of its $2^{\mathrm{n}}$ input lines is active, i.e. at logic 1 . A [[keyboard encoder]], for example, may be required to generate a unique binary code indicating which key on the keyboard has been pressed.
...
## Source[^2]
- A device or circuit that produces an output in a desired coded form.
## References
