...
## Source[^8]
- A digraph in which every arc is assigned a weight (normally some non-negative number). In some applications, something may be thought of as flowing or being transported between the vertices of a network, with the weight of each arc giving its capacity. In some other cases, the vertices of a network may represent steps in a process and the weight of the arc joining $u$ and $v$ may give the time that must elapse between step $u$ and step $v$.
## References
