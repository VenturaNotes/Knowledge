...
- The function $s$ can be extended to map $\Sigma_{1}$-words to $\Sigma_{2}$-languages:$$s(a_{1} \ldots a_{n})=s\left(a_{1}\right) \ldots s\left(a_{n}\right)$$i.e. the concatenation of the languages $s(a_{1}), \ldots, s(a_{n}) . s$ can then be further extended to map $\Sigma_{1}$-languages to $\Sigma_{2}$-languages:$$s(L)=\{s(w) \mid w \in L\}$$$s(L)$ is called the substitution image of $L$ under s. 
