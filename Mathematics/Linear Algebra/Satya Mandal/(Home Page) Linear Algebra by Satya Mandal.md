---
status: open
Source:
  - https://mandal.ku.edu/math290/
  - zotero://open-pdf/library/items/YU4BNLTR?page=1&annotation=79Z4YEK3
Length: "804"
tags:
  - type/slides
---
- Complete
- Incomplete
	- [[(1.1) Introduction to linear Equations]]
	- [[(1.2) Gaussian Elimination]]
	- [[(1.3) Application of Linear Systems]]
	- [[(2.1) Operations with Matrices]]
	- [[(2.2) Properties of Matrices]]
	- [[(2.3) The Inverse of Matrices]]
	- [[(2.4) Elementary Matrices]]
	- [[(3.1) Determinant]]
	- [[(3.2) Evaluation of Determinant, with Elementary Operations]]
	- [[(3.3) Properties of Determinants]]
	- [[(3.4) Applications of Determinants]]
	- [[(4.1) Vectors in n-spaces]]
	- [[(4.2) Vector Spaces]]
	- [[(4.3) Subspaces of a Vector Space]]
	- [[(4.4) Spanning and Independence]]
	- [[(4.5) Basis and Dimension]]
	- [[(4.6) Rank of a Matrix II]]
	- [[(5.1) Eigenvalues and Eigenvectors]]
	- [[(5.2) Diagonalization]]
	- [[(6.1) Length and Dot Product]]
	- [[(6.2) Inner Product Spaces]]
	- [[(6.3) Orthonormal Basis]]
	- [[(7.1) Linear Transformation - Definitions and Introduction]]
	- [[(7.2) Properties of Linear Transformations]]