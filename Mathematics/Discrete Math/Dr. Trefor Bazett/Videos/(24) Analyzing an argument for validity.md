---
Source:
  - https://youtube.com/watch?v=ON7yAw6W9VY
Reviewed: false
---
- ![[Screenshot 2023-05-09 at 11.23.11 PM.png]]
	- Example
		- If I'm skilled at poker, then I will win.
		- I won money playing poker.
		- Therefore, I'm skilled at poker
	- Argument does not work because we are trying to use the converse of the statement (which is not logically equivalent) to state that the argument is valid (which it is not)
	- Invalid logical structure