...
---
## Feature Requests
- [ ] Fix "Default" mode so that it tells you how much time is left to complete everything
- [ ] Need to test if the "sections" feature works
## Description
- The shortcut works so whichever hotkey used to open "Open Sections Panel", the shortcut will also be used to close it
## V9
- Made it so there is a recalculation if I'm behind. Seems to work pretty well. Didn't test all features but seems good enough
	- For example, if I'm behind in time and i have 46s left, it tells me i actually need to finish in 40s so that I will be able to complete on time! 
		- Although I do wonder if it still subtracts from the "behind vs forward" part but we'll just see....
## V8
- Added a "Timed" Mode
```ts
...
interface PluginData {
	...
	[notePath: string]: NoteSession | boolean | undefined;
}
...
function fmtAbs(ms: number): string {
	const isNeg = ms < 0;
	const str = fmt(Math.abs(ms));
	return isNeg ? `-${str}` : str;
}

function fmtSigned(ms: number): string {
	if (Math.round(ms / 1000) === 0) return "0s";
	const isNeg = ms < 0;
	const str = fmt(Math.abs(ms));
	return isNeg ? `-${str}` : `+${str}`;
}
...
function parseDuration(input: string): number | null {
	...
	if (hMatch || mMatch || sMatch) {
		const hours = hMatch && hMatch[1] ? parseInt(hMatch[1]) : 0;
	...
	// Support word format "X hour", "Y hours"
	const hourWordMatch = input.match(/(\d+)\s*hour/);
	if (hourWordMatch && hourWordMatch[1]) {
		return parseInt(hourWordMatch[1]) * 3600 * 1000;
	}
...
class PromptModal extends Modal {
	...
	constructor(app: App, placeholder: string, defaultValue: string, onSubmit: (result: string) => void) {
		super(app);
	...
	onOpen() {
		...
		contentEl.createEl("h2", { text: "Set Target Duration" });
...
class SectionsModal extends Modal {
	...
	constructor(app: App, plugin: ReadingPacePlugin, view: MarkdownView, editor: Editor) {
		...
		this.showIncompleteOnly = !!this.plugin.data.showIncompleteOnly;
	}
	...
	onOpen() {
		...
		// Mode button
		const getModeBtnText = () => {
			const sess = this.plugin.getSession(this.path);
			return sess?.mode === "timed" ? `🎯 Timed (${fmt(sess.targetDurationMs ?? 0)})` : "⏱ Default";
		};
		const modeBtn = titleRow.createEl("button", { text: getModeBtnText() });
		modeBtn.style.cssText = "font-size:11px; padding:3px 10px; border-radius:4px; border:1px solid var(--background-modifier-border); background:var(--background-secondary); color:var(--text-normal); cursor:pointer;";
		modeBtn.addEventListener("click", async () => {
			const sess = this.plugin.getSession(this.path);
			if (!sess) return;
			const newMode = sess.mode === "timed" ? "default" : "timed";
			if (newMode === "timed") {
				const currentDefault = fmt(sess.targetDurationMs || 3600000);
				new PromptModal(
					this.app,
					"e.g. '1 hour', '1h 30m', '45m'",
					currentDefault,
					async (val) => {
						const ms = parseDuration(val);
						if (ms === null || ms <= 0) {
							new Notice("Invalid duration format.");
							return;
						}
						sess.targetDurationMs = ms;
						sess.mode = "timed";
						await this.plugin.saveSessionData();
						modeBtn.textContent = getModeBtnText();
						this.renderList();
		...
		filterBtn.addEventListener("click", async () => {
			...
			this.plugin.data.showIncompleteOnly = this.showIncompleteOnly;
			await this.plugin.saveSessionData();
			updateFilterBtn();
			this.renderList();
		});
		...
		// Feature 3: Command/Ctrl + Enter marks running section as done and starts next
		this.scope.register(["Mod"], "Enter", async (evt) => {
			evt.preventDefault();
			const activeIdx = this.plugin.getActiveSegmentIdx(this.path);
			if (activeIdx !== null) {
				await this.plugin.toggleSegmentComplete(this.path, activeIdx);
				const sess = this.plugin.getSession(this.path);
				if (sess) {
					const nextIdx = activeIdx + 1;
					if (nextIdx < sess.segments.length) {
						await this.plugin.startSegment(this.path, nextIdx);
					}
				}
				this.renderList();
			}
		...
		// Feature 4: Close modal on designated command hotkey or Command + Shift + H fallback
		const commandId = `${this.plugin.manifest.id}:open-sections`;
		let hotkeys = (this.app as any).hotkeyManager?.getHotkeys?.(commandId);
		if (!hotkeys || hotkeys.length === 0) {
			hotkeys = (this.app as any).commands?.commands?.[commandId]?.hotkeys;
		}
		...
		if (hotkeys && Array.isArray(hotkeys)) {
			...
			this.scope.register(["Mod", "Shift"], "H", (evt) => {
				evt.preventDefault();
				this.close();
			});
		}
	...
	private renderList() {
		...
		session.segments.forEach((seg, idx) => {
			...
			if (isTimed) {
				const alloc = this.plugin.getSegmentAllocation(session, idx);
				const spent = this.plugin.liveSegmentMs(this.path, idx);
				if (isActive) {
					durText = `${fmt(spent)} / ${fmt(alloc)}`;
				} else if (isDone) {
					durText = `${fmt(spent)} / ${fmt(alloc)}`;
				} else {
					durText = fmt(alloc);
				...
				durText = liveMs > 0 ? fmt(liveMs) : "";
			}

			const durEl = row.createEl("span", { text: durText });
			durEl.style.cssText = "font-size:12px; color:var(--text-muted); flex-shrink:0; width:80px; text-align:right;";
			
			const liveMs = this.plugin.liveSegmentMs(this.path, idx);
			if (isTimed || liveMs > 0) {
				durEl.style.cursor = "pointer";
				durEl.title = "Click to edit spent time";
	...
	private tickLive() {
		...
		const isTimed = session.mode === "timed";
		if (isTimed) {
			const alloc = this.plugin.getSegmentAllocation(session, activeIdx);
			const spent = this.plugin.liveSegmentMs(this.path, activeIdx);
			durEl.textContent = `${fmt(spent)} / ${fmt(alloc)}`;
		} else {
...
export default class ReadingPacePlugin extends Plugin {
	...
	activeModal: SectionsModal | null = null;
	...
	async onload() {
		...
		for (const path of Object.keys(this.data)) {
			if (path === "showIncompleteOnly") continue;
			const session = this.data[path] as NoteSession;
		...
		this.addCommand({
			...
			editorCallback: (editor: Editor, view: MarkdownView | MarkdownFileInfo) => {
				...
				if (this.activeModal) {
					...
					this.activeModal = new SectionsModal(this.app, this, mv, editor);
		...
		this.registerEvent(
			this.app.vault.on("rename", (file, oldPath) => {
				if (oldPath === "showIncompleteOnly") return;
	...
	getSession(path: string): NoteSession | undefined {
		...
		if (session && typeof session === "object" && "segments" in session) {
	...
	async stopActiveSegment(path: string) {
		...
		const session = this.getSession(path);
		const seg = session?.segments[activeIdx];
		const start = this.activeWindowStart[path];
	...
	getSegmentAllocation(session: NoteSession, idx: number): number {
		const totalWords = session.segments.reduce((sum, s) => sum + s.wordCount, 0);
		if (totalWords === 0) return 0;
		const target = session.targetDurationMs ?? 0;
		const seg = session.segments[idx];
		if (!seg) return 0;
		return Math.round(target * (seg.wordCount / totalWords));
	}
	...
	private ensureSession(view: MarkdownView) {
		...
		if (!this.getSession(path)) {
			this.data[path] = { 
				segments: buildSegments(view.editor.getValue()),
				mode: "default"
			};
		}
	...
	private async stampAtCursor(editor: Editor, view: MarkdownView) {
		...
		const isTimed = session.mode === "timed";
		if (isTimed) {
			const incompleteSegments = session.segments.filter(s => !s.completedAt);
			const totalIncompleteAllocMs = incompleteSegments.reduce((sum, s) => {
				const sIdx = session.segments.indexOf(s);
				return sum + this.getSegmentAllocation(session, sIdx);
			}, 0);
			let netSavedCompleted = 0;
			session.segments.forEach((seg, sIdx) => {
				if (seg.completedAt) {
					const allocated = this.getSegmentAllocation(session, sIdx);
					netSavedCompleted += (allocated - seg.durationMs);
				}
			});
			new Notice(`✓ ${seg.heading}\nRemaining: ${fmtAbs(totalIncompleteAllocMs)}\nAhead/Behind: ${fmtSigned(netSavedCompleted)}`);
		} else {
	...
	refreshStatus() {
		...
		if (activePath !== null) {
			const activeSess = this.getSession(activePath);
			...
			if (activeSess && activeIdx !== null && activeIdx !== undefined) {
				...
				const isTimed = activeSess.mode === "timed";
				if (isTimed) {
					// 1. Calculate sum of pre-calculated target allocations of remaining incomplete headings
					const incompleteSegments = activeSess.segments.filter(s => !s.completedAt);
					const totalIncompleteAllocMs = incompleteSegments.reduce((sum, s) => {
						const sIdx = activeSess.segments.indexOf(s);
						return sum + this.getSegmentAllocation(activeSess, sIdx);
					}, 0);

					// 2. Base calculation of completed save/overrun
					let netSavedCompleted = 0;
					activeSess.segments.forEach((seg, sIdx) => {
						if (seg.completedAt) {
							const allocated = this.getSegmentAllocation(activeSess, sIdx);
							netSavedCompleted += (allocated - seg.durationMs);
						}
					});

					// 3. Current active segment details
					const allocatedActive = this.getSegmentAllocation(activeSess, activeIdx as number);
					const spentActive = activeMs;
					const timeLeftActive = allocatedActive - spentActive;

					// 4. Overrun triggers real-time countdown on ahead/behind metric
					const overrunActive = timeLeftActive < 0 ? Math.abs(timeLeftActive) : 0;
					const totalAheadBehind = netSavedCompleted - overrunActive;

					this.statusBar.setText(
						`📖 ${activeSeg?.heading ?? ""} | Est. remaining: ${fmtAbs(totalIncompleteAllocMs)} | ${fmtSigned(totalAheadBehind)} | ${fmtAbs(timeLeftActive)}`
					);
				} else {
		...
		if (isTimed) {
			...
			this.statusBar.setText(`📖 Est. remaining: ${fmtAbs(totalIncompleteAllocMs)} | ${fmtSigned(netSavedCompleted)}`);
		} else {
