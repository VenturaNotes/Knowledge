...
## V6
- Simplified the UI
- Added a new feature in "Stacking Mode" so that it just uses the "global default duration"
```javascript
import Cocoa
import SwiftUI
...
class MenuBarController {
    ...
    private let stackingUseGlobalDurationKey = "StackingUseGlobalDuration"
    ...
    // When true, stacking mode uses countdownDuration for every level instead of the computed avg.
    private var stackingUseGlobalDuration: Bool {
        get { return UserDefaults.standard.bool(forKey: stackingUseGlobalDurationKey) }
        set { UserDefaults.standard.set(newValue, forKey: stackingUseGlobalDurationKey) }
    }
    ...
    @objc private func manageCycle() {
        ...
        let container = NSView(frame: NSRect(x: 0, y: 0, width: 360, height: 322))
        ...
        // Row 1: Cycle labels field (top)
        let labelsField = NSTextField(frame: NSRect(x: 0, y: 298, width: 360, height: 24))
        ...
        indexLabel.frame = NSRect(x: 0, y: 274, width: 180, height: 17)
        let indexField = NSTextField(frame: NSRect(x: 185, y: 272, width: 60, height: 22))
        indexField.stringValue = "\(cycleIndex)"

        // Row 3: Global default duration
        let globalDurLabel = NSTextField(labelWithString: "Global default duration:")
        globalDurLabel.frame = NSRect(x: 0, y: 248, width: 180, height: 17)
        let globalDurField = NSTextField(frame: NSRect(x: 185, y: 246, width: 80, height: 22))
        ...
        goalLabel.frame = NSRect(x: 0, y: 222, width: 210, height: 17)
        let goalField = NSTextField(frame: NSRect(x: 215, y: 220, width: 80, height: 22))
        goalField.stringValue = formatDuration(stackingGoal)
        goalField.placeholderString = "e.g. 15m"

        // Row 5: Positive-only checkbox
        ...
        positiveOnlyCheckbox.frame = NSRect(x: 0, y: 196, width: 360, height: 22)
        positiveOnlyCheckbox.state = stackingGoalPositiveOnly ? .on : .off

        // Row 6: Stacking uses global duration checkbox
        let stackingGlobalDurCheckbox = NSButton(checkboxWithTitle: "Stacking Mode: Use Global Default Duration", target: nil, action: nil)
        stackingGlobalDurCheckbox.frame = NSRect(x: 0, y: 170, width: 360, height: 22)
        stackingGlobalDurCheckbox.state = stackingUseGlobalDuration ? .on : .off

        // Row 7: Stacking upper limit
        let upperLimitLabel = NSTextField(labelWithString: "Stacking upper limit (blank = none):")
        upperLimitLabel.frame = NSRect(x: 0, y: 144, width: 220, height: 17)
        let upperLimitField = NSTextField(frame: NSRect(x: 225, y: 142, width: 80, height: 22))
        ...
        // Row 8: Countdown target
        let cdTargetLabel = NSTextField(labelWithString: "Countdown target (# of presses):")
        cdTargetLabel.frame = NSRect(x: 0, y: 118, width: 220, height: 17)
        let cdTargetField = NSTextField(frame: NSRect(x: 225, y: 116, width: 80, height: 22))
        cdTargetField.stringValue = "\(countdownTarget)"
        cdTargetField.placeholderString = "e.g. 10"

        // Row 9: Show overlay checkbox
        let overlayCheckbox = NSButton(checkboxWithTitle: "Show Cycle Label on Press", target: nil, action: nil)
        overlayCheckbox.frame = NSRect(x: 0, y: 90, width: 360, height: 22)
        overlayCheckbox.state = showCycleLabelOverlay ? .on : .off

        // Notifications section header
        let notificationsHeader = NSTextField(labelWithString: "Notifications")
        notificationsHeader.frame = NSRect(x: 0, y: 64, width: 360, height: 17)
        notificationsHeader.font = NSFont.boldSystemFont(ofSize: NSFont.smallSystemFontSize)
        notificationsHeader.textColor = .secondaryLabelColor

        // Row 10: Counter Threshold
        let counterThresholdLabel = NSTextField(labelWithString: "Counter threshold (# of presses):")
        counterThresholdLabel.frame = NSRect(x: 0, y: 42, width: 225, height: 17)
        let counterThresholdField = NSTextField(frame: NSRect(x: 230, y: 40, width: 80, height: 22))
        counterThresholdField.stringValue = "\(notificationThreshold)"
        counterThresholdField.placeholderString = "e.g. 30"

        // Row 11: Time Threshold (bottom)
        let timeThresholdLabel = NSTextField(labelWithString: "Time threshold (minutes):")
        timeThresholdLabel.frame = NSRect(x: 0, y: 14, width: 210, height: 17)
        let timeThresholdField = NSTextField(frame: NSRect(x: 215, y: 12, width: 80, height: 22))
        timeThresholdField.stringValue = "\(timeNotificationThreshold / 60)"
        timeThresholdField.placeholderString = "e.g. 30"
        ...
        container.addSubview(stackingGlobalDurCheckbox)
        ...
        container.addSubview(notificationsHeader)
        container.addSubview(counterThresholdLabel)
        container.addSubview(counterThresholdField)
        container.addSubview(timeThresholdLabel)
        container.addSubview(timeThresholdField)
        ...
        presentAlert(alert, firstResponder: labelsField) { [weak self] response in
            ...
            self.stackingUseGlobalDuration = (stackingGlobalDurCheckbox.state == .on)
            ...
            let counterThresholdInput = counterThresholdField.stringValue.trimmingCharacters(in: .whitespacesAndNewlines)
            if let newValue = Int(counterThresholdInput), newValue > 0 {
                self.notificationThreshold = newValue
            }

            let timeThresholdInput = timeThresholdField.stringValue.trimmingCharacters(in: .whitespacesAndNewlines)
            if let newMinutes = Int(timeThresholdInput), newMinutes > 0 {
                self.timeNotificationThreshold = newMinutes * 60
            }
    ...
    private func performLevelTransition(direction: Int) {
        ...
        if stackingUseGlobalDuration {
            // Toggle on: always use the global default duration regardless of computed avg
