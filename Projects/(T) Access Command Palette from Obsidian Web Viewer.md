...
- [ ] Make the script work on pdf sites
	- [https://www.softouch.on.ca/kb/data/Concise Oxford Dictionary of Mathematics.pdf](https://www.softouch.on.ca/kb/data/Concise%20Oxford%20Dictionary%20of%20Mathematics.pdf)
## Conversations
...
## V4 Solution
- Added memory leak prevention in-case the script is run twice in a row
- Prevent from being able to switch between tabs easily

```javascript
/**
 * WebviewCommands.js
 * Version: 3.1 - Safe Focus Yielding
 */
...
module.exports = async (params) => {
    ...
    // --- 1. THE COMMAND SHIELD (Memory-Leak Free) ---
    const lastFired = new Map();
        ...
        webview.addEventListener('console-message', (e) => {
            ...
            if (e.message?.startsWith('OBS_RAW_KEY:')) {
                ...
                if (commandId) {
                    ...
                    // FIX: Check if the native hotkey already changed the active tab (like TabNext does).
                    // If we are no longer on the webview tab, abort entirely and let the new tab keep focus!
                    if (app.workspace.activeLeaf !== leaf) {
                        return;
                    }
...
## V3 Solution
- Modified script so that the shortcut hotkeys only update when switching views (and only populated when in a web viewer. Still works and makes obsidian much faster)
```javascript
/**
 * WebviewCommands.js
...
```
## V2 Solution
- Tabs generally movement resilient (at least for left and right panes)
	- Doesn't seem to work too well with 3 panes, but not part of my use case
	- Whenever a document such a google docs would be manually moved to a new pane, Obsidian shortcuts would no longer function. A quick hack was developed so the moved tab to the new pane would be duplicated and then deleted so the duplicate would at least have the obsidian shortcut functionality. This may be an electron issue.
- Works with inactive web viewer tabs and silenced several errors 
```javascript
/**
 * WebviewCommands.js
...
```
## Old Solution: JavaScript QuickAdd Strategy
```javascript
/**
 * WebviewCommands.js
