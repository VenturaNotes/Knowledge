...
- Capital X: $\overline{\underline X}$ 
	...
	- Could do $\bar X$ for a bar on top
- tau: $\uptau$ 
...
- Limit: $\lim_{x\to\infty} f(x)$
	...
	- $\iiint\limits_{D} \rho(x,y,z)\,dx\,dy\,dz$
		- Can also use `limits_` $\int \limits_{3}$ so subscript appears underneath the integral symbol
- To: $\to$ 
- Sizes
	- Huge: $\Huge \in$
	- huge: $\huge \in$ 
...
- Real part of complex plane (horizontal axis): $\Re$ or $\mathfrak{R}$ 
...
- Power set: $\wp$ 
	...
	- Seems like $\mathcal{P}$ also valid
...
- Mod
	- $\bmod$
	- This renders the binary modulus operator
- Quotation Marks (2 methods)
	- $\text{`good'}$
		- Use backtick
	- $\textquoteleft\text{good'}$ 
		- Use `\textquoteleft`
- Trigonometry
	- $\sin$
	- $\cos$
- Reduced Planck's constant
	- $\hbar = \frac{h}{2\pi}$ 
- Properly size parentheses
	- $$\left(\int_3^4 + \int_6^4\right)$$
- Absolute Value
	- $\lvert n \rvert$
		- If the $|n|$ doesn't work.
- Counterfactual conditional 
	- $\Box\!\!\rightarrow$ or $\Box\kern -0.4em\rightarrow$ 
		- Better to use `kern` with `em` as it will scale if your text scales (which `pt`) does not do

## Not Supported
- Part of Gauss's Flux Theorem (integral form)
	- $\oiint$