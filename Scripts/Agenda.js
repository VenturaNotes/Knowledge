...
// Initialized to the start of the month to prevent rollover bugs (e.g., being on the 31st of a month and skipping February)
let currentMoment = window.moment().startOf('month'); 
...
.v7-month-title { font-size: 1.3rem; font-weight: bold; cursor: pointer; display: flex; align-items: center; gap: 8px; transition: color 0.1s; border-radius: 4px; padding: 2px 6px; }
.v7-month-title:hover { color: var(--text-accent); background: var(--background-modifier-hover); }
.v7-select { background: var(--background-secondary); color: var(--text-normal); border: 1px solid var(--background-modifier-border); border-radius: 4px; font-size: 0.9rem; padding: 2px 6px; cursor: pointer; font-family: var(--font-interface); }
.v7-select:focus { border-color: var(--interactive-accent); outline: none; }
...
async function render(params, leaf) {
    ...
    // Renders the clickable month title
    const monthTitle = nav.createDiv({ cls: "v7-month-title", text: currentMoment.format("MMMM YYYY") });
    
    monthTitle.onclick = () => {
        monthTitle.onclick = null; // Prevent double-triggering click events while editing
        monthTitle.empty();
        
        // Month Selector
        const monthSelect = monthTitle.createEl("select", { cls: "v7-select" });
        for (let i = 0; i < 12; i++) {
            const opt = monthSelect.createEl("option", { value: i, text: window.moment().month(i).format("MMMM") });
            if (i === currentMoment.month()) opt.selected = true;
        }
        
        // Year Selector (Current year +/- 10 years)
        const yearSelect = monthTitle.createEl("select", { cls: "v7-select" });
        const baseYear = window.moment().year();
        const startYear = baseYear - 10;
        const endYear = baseYear + 10;
        for (let y = startYear; y <= endYear; y++) {
            const opt = yearSelect.createEl("option", { value: y, text: String(y) });
            if (y === currentMoment.year()) opt.selected = true;
        }

        // Control Buttons
        const btnContainer = monthTitle.createSpan({ style: "display: flex; gap: 4px; margin-left: 5px;" });
        const okBtn = btnContainer.createEl("button", { cls: "v7-btn", text: "✓" });
        const cancelBtn = btnContainer.createEl("button", { cls: "v7-btn", text: "✕", style: "background: var(--background-modifier-border); color: var(--text-normal);" });

        okBtn.onclick = (e) => {
            e.stopPropagation();
            // Safely pin to the 1st of the targeted month/year to prevent date overflow errors
            currentMoment.date(1).month(parseInt(monthSelect.value)).year(parseInt(yearSelect.value));
            render(params, leaf);
        };

        cancelBtn.onclick = (e) => {
            e.stopPropagation();
            render(params, leaf); // Re-render to restore text mode
        };
