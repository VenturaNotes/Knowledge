...
async function collectData(app) {
    ...
    const parseTime = (tStr) => {
        ...
        const m = window.moment(tStr, ["HH:mm", "h:mmA", "h:mm A", "hh:mm a"]);
        return m.isValid() ? m.format("HH:mm") : "00:00";
    };

    // Helper to strip dates and times from text
    const cleanText = (t) => t.replace(/📅\s*\d{4}-\d{2}-\d{2}/g, "").replace(/⏰\s*\d{1,2}:\d{2}(?:\s?[APMapm]{2})?/g, "").trim();

    for (const file of files) {
        const cache = app.metadataCache.getFileCache(file);
        const fm = cache?.frontmatter || {};
        const tags = Array.isArray(fm.tags) ? fm.tags : [fm.tags];
        if (!tags.some(t => String(t).toLowerCase().includes("task"))) continue;
        ...
        lines.forEach((l, i) => {
            ...
            if (m) {
                ...
                // Adjustment: Only include if the checkbox has its own specific 📅 date
                if (!dM) return; 
                ...
                allItems.push({
                    ...
                    status, 
                    date: dM[1], 
                    rrule: null, // Checkboxes with dates don't inherit file-level recurrence here
...
function createItemEl(item, app, isPanel = false) {
    ...
    const time = (item.time === "00:00") ? "" : item.time;
    
    if (isPanel) {
        el.innerHTML = `<div style="font-size:0.7rem; color:var(--text-accent)">${time} ${item.isProject ? '◈ PROJECT' : '○ TASK'}</div>
                        <div style="font-weight:bold">${isDone ? '✓ ' : ''}${item.text}</div>
                        <div style="font-size:0.6rem; opacity:0.5; margin-top:5px">${item.file}</div>`;
    } else {
        el.innerHTML = `<span>${isDone ? '✓ ' : ''}${time} ${item.text}</span>`;
    }
...
async function render(params, leaf) {
    ...
    ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].forEach(d => grid.createDiv({ cls: "v7-weekday", text: d }));

    const startOfGrid = currentMoment.clone().startOf('month').startOf('week');
    const endOfGrid = currentMoment.clone().endOf('month').endOf('week');
    ...
    if (selectedDateItems) {
        ...
        if (selectedDateItems.length === 0) pContent.createDiv({text: "No tasks for this day.", style: "opacity:0.5"});
        selectedDateItems.forEach(i => pContent.appendChild(createItemEl(i, app, true)));
    }
}
